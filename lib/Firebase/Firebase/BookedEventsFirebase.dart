...
class BookedEventsFirebase {
  //add appointment
  bookEvent(Client client, BookingEvent bookingEvent, String name,
  ...
  Future<List<Meeting>> getBookEventsForClient(
          ...
          for (var docSnapshot in value.docs)
            {
              ...
              meetings.add(Meeting(
                  "Lash Appointment for ${docSnapshot.data().firstName}. Phone: ${docSnapshot.data().phoneNumber}",
                  docSnapshot.data().startTime,
                  docSnapshot.data().startTime,
  ...
  completeAppointment(BookingEvent bookingEvent) async {
    final docRef = FirebaseFirestore.instance
        .collection('bookEvents').doc(bookingEvent.id)
        .withConverter(
        fromFirestore: BookingEvent.fromFireStore,
        toFirestore: (BookingEvent bookingEvent, options) =>
            bookingEvent.toFireStore());
  ...
  approveAppointment(BookingEvent bookingEvent) async {
    final docRef = FirebaseFirestore.instance
        .collection('bookEvents').doc(bookingEvent.id)
        .withConverter(
        fromFirestore: BookingEvent.fromFireStore,
  ...
  deleteAppointment(BookingEvent bookingEvent) async {
    final docRef = FirebaseFirestore.instance
        .collection('bookEvents').doc(bookingEvent.id);
